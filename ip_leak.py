...
def get_ip_leak_data():
    ...
    for _ in range(15):
        ...
        json_response = response.json()
        dns_data = {'ip': json_response['ip'], 'region':json_response['region_name'], 'country':json_response['country_name']}
    ...
    for i in ip_leak_json['dns_info']:
        if i['country'] != ip_leak_json['ip_info']['country']:
            discrepancies = "IP addresses resolved to different locations. {} resolved to {}".format(i['country'],  ip_leak_json['ip_info']['country'])
